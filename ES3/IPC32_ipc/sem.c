    ...
    while (choice != 'q')
    {
        ...
        switch (choice)
        {
            case 'n':
				...
				printf("Enter initial value (default 1): ");
				fgets(line, sizeof(line), stdin);
				sscanf(line, "%i", &initial_value);

				printf("Enter permission [octal](default 0600): ");
				fgets(line, sizeof(line), stdin);
				sscanf(line, "%i", &permissions);

                printf ("Calling sem_open('%s', O_CREAT | O_EXCL, %i, %i)\n", sem_name, permissions, initial_value);
				
                semdes = sem_open (sem_name, O_CREAT | O_EXCL, octal_decimal(permissions), initial_value);
